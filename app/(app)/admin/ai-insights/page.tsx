...
import { Bot, Lightbulb, TrendingUp, Users, Target, RefreshCw, Printer } from "lucide-react";
...
export default function AiInsightsPage() {
  ...
  return <div className="ai-report mx-auto max-w-6xl px-4 py-6">
    <style jsx global>{`@media print { aside, header, .no-print { display: none !important; } body { background: white !important; } .ai-report { max-width: none !important; padding: 0 !important; } .card { break-inside: avoid; box-shadow: none !important; } a { color: inherit !important; text-decoration: none !important; } }`}</style>
    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between no-print">
      ...
      <div className="flex gap-2"><select className="input" value={days} onChange={(e) => setDays(Number(e.target.value))}><option value={30}>直近30日</option><option value={60}>直近60日</option><option value={90}>直近90日</option></select><button className="btn-ghost" onClick={load}><RefreshCw className="h-4 w-4" />更新</button><button className="btn-primary" onClick={() => window.print()}><Printer className="h-4 w-4" />PDF保存・印刷</button></div>
    </div>
    {!data ? <div className="card mt-6 p-6 text-sm text-gray-500">分析データを取得できませんでした。</div> : <>
      <section className="hidden print:block mt-2 border-b pb-4"><p className="text-sm text-gray-500">Rizbo 運用改善レポート</p><h1 className="mt-1 text-2xl font-bold">{reportName}｜集客・体験予約レポート</h1><p className="mt-2 text-sm text-gray-600">対象期間：直近{data.days}日 ／ 作成日：{new Date(data.generatedAt).toLocaleDateString("ja-JP")}</p></section>
