...
export default function AiInsightsClient({ initialInsight = null }: { initialInsight?: Insight | null }) {
  ...
  const funnelLinePoints = useMemo(() => {
    const rowHeight = 10;
    const rowGap = 8;
    ...
    return (data?.funnel ?? []).map((item, index) => {
      ...
      const y = ((rowHeight / 2 + index * (rowHeight + rowGap)) / totalHeight) * 100;
      return `${x},${y}`;
    }).join(" ");
  }, [data, funnelMax]);
