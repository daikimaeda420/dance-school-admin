...
function getStatus(ok: boolean, warn = false): Status {
...
function countLinkedItems(
  ids: string[],
  linksById: Map<string, Set<string>>,
) {
  return ids.filter((id) => (linksById.get(id)?.size ?? 0) > 0).length;
}

async function loadChecklistData(schoolId: string) {
  const [
    campuses,
    courses,
    lifestyles,
...
export default async function DiagnosisChecklistPage({
  searchParams,
}: {
  ...
  const data = await loadChecklistData(schoolId);
  ...
  const completionPercent = Math.round((okCount / allItems.length) * 100);
  const summaryStatus: Status =
    blockingCount > 0 ? "missing" : warnCount > 0 ? "warn" : "ok";
  const SummaryIcon = STATUS_META[summaryStatus].icon;

  const stats: Stat[] = [
    {
      label: "完成度",
      value: `${completionPercent}%`,
      caption: `${okCount}/${allItems.length}項目 OK`,
    },
    {
      label: "未設定",
      value: `${blockingCount}`,
    ...
    {
      label: "有効データ",
      value: `${activeCampuses.length}/${activeCourses.length}/${activeInstructors.length}`,
      caption: "校舎 / コース / 講師",
    },
  ];
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        ...
        <div
          className={[
            ...
            STATUS_META[summaryStatus].badgeClass,
          ].join(" ")}
        >
