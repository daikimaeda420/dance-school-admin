...
export default function ChatReservationsPage() {
  ...
  return <div className="mx-auto max-w-[1540px] px-4 py-6 text-slate-800 md:px-6">
    ...
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-900"><p>新しい予約希望は、フォーム設定の「スクールへの通知先」メールアドレスにも届きます。</p><Link href={`/admin/diagnosis/form?schoolId=${encodeURIComponent(schoolId ?? "")}`} className="shrink-0 text-xs font-semibold text-blue-700 hover:underline">通知先メールを確認する →</Link></div>
