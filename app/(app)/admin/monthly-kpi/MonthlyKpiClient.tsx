...
function RevenueChart({ months, metric }: { months: Month[]; metric: Metric }) {
  ...
  const xFor = (index: number) => 52 + index * (296 / Math.max(1, values.length - 1));
  ...
  return <div className="relative mt-4 overflow-x-auto"><div className="mb-1 flex justify-end gap-4 text-[11px] font-semibold"><span className="flex items-center gap-1.5 text-[#fe6147]"><i className="h-0.5 w-4 bg-[#fe6147]" />月次 推定売上</span>{showCumulative && <span className="flex items-center gap-1.5 text-blue-700"><i className="w-4 border-t-2 border-dashed border-blue-600" />累計 推定売上</span>}</div><svg viewBox="0 0 400 196" className="min-w-[620px] w-full" role="img" aria-label={`${labelFor[metric]}の月次推移`}>
    ...
    {showCumulative && <polyline points={cumulativePoints} fill="none" stroke="#54708f" strokeWidth="2" strokeDasharray="5 5" strokeLinejoin="round" strokeLinecap="round" />}
    {values.map((value, index) => { const x = xFor(index); const y = 158 - value / max * 112; const cumulativeY = 158 - cumulativeValues[index] / cumulativeMax * 112; return <g key={months[index].key}><circle cx={x} cy={y} r="4.5" fill="#fe6147" stroke="white" strokeWidth="2" /><text x={x} y={y - 10} textAnchor="middle" fontSize="9" fontWeight="700" fill="#475569">{display(value)}</text>{showCumulative && <><circle cx={x} cy={cumulativeY} r="3.5" fill="white" stroke="#54708f" strokeWidth="1.5" /><text x={x + 6} y={cumulativeY + 11} fontSize="8" fontWeight="700" fill="#54708f">{formatYen(cumulativeValues[index])}</text></>}<text x={x} y="180" textAnchor="middle" fontSize="10" fill="#64748b">{months[index].label}</text></g>; })}
    {showCumulative && [0, 1, 2, 3].map((step) => <text key={step} x="372" y={160 - step * 37} fontSize="8" fill="#64748b">{formatYen(Math.round(cumulativeMax * step / 3))}</text>)}
  </svg></div>;
}
