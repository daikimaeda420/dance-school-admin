...
export default function AiInsightsClient({ initialInsight = null }: { initialInsight?: Insight | null }) {
  ...
  return <div className="mx-auto max-w-[1540px] px-4 py-6 text-slate-800 md:px-6">
    ...
    <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
      ...
      <article className="rounded-xl border border-orange-200 bg-gradient-to-br from-orange-50 to-white p-4 shadow-sm"><p className="text-sm font-semibold text-[#c53f2b]">AI改善スコア</p><div className="mt-2 flex items-end gap-2"><strong className="text-4xl text-[#fe6147]">{improvementScore}</strong><span className="mb-1 text-sm text-slate-500">/ 100</span></div><p className="mt-2 text-xs text-slate-500">提案を適用して、効果を積み上げましょう。</p><button type="button" onClick={() => setScoreDetailOpen(true)} className="mt-3 inline-flex w-full items-center justify-center gap-1.5 rounded-md border border-orange-200 bg-white px-3 py-2 text-xs font-semibold text-[#c53f2b] transition hover:bg-orange-50">スコアの詳細を見る <ChevronRight className="h-3.5 w-3.5" /></button></article>
    </section>
    ...
    {scoreDetailOpen && <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-900/40 p-4" role="dialog" aria-modal="true" aria-labelledby="score-detail-title"><div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="flex items-center gap-2 text-sm font-semibold text-[#fe6147]"><Sparkles className="h-4 w-4" />AI改善スコア</p><h2 id="score-detail-title" className="mt-1 text-xl font-bold text-slate-900">{improvementScore} / 100 の内訳</h2><p className="mt-1 text-sm text-slate-500">直近{data.days}日の指標を、目標値に対して4項目・各25点で評価しています。</p></div><button aria-label="閉じる" onClick={() => setScoreDetailOpen(false)} className="rounded-md p-1 text-slate-400 hover:bg-slate-100"><X className="h-5 w-5" /></button></div><div className="mt-5 space-y-3">{scoreBreakdown.map((item) => <article key={item.key} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-4"><div><h3 className="text-sm font-bold text-slate-800">{metricLabels[item.key]}</h3><p className="mt-1 text-xs text-slate-500">{item.note}</p></div><strong className="text-lg text-[#fe6147]">{item.score}<span className="text-xs font-medium text-slate-400"> / 25点</span></strong></div><div className="mt-3 flex items-center gap-3"><div className="h-2 flex-1 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-[#fe6147]" style={{ width: `${item.value == null ? 0 : Math.min(100, item.value / item.target * 100)}%` }} /></div><span className="w-28 text-right text-xs font-semibold text-slate-700">{rate(item.value)} <span className="font-normal text-slate-400">/ 目標{item.target}%</span></span></div></article>)}</div><div className="mt-5 rounded-xl border border-orange-200 bg-orange-50 p-4"><p className="text-sm font-bold text-[#9a3525]">次に優先する改善</p>{data.suggestions.slice(0, 2).map((suggestion) => <div key={suggestion.title} className="mt-2 flex items-center justify-between gap-3 text-sm"><span className="text-[#9a3525]">{suggestion.title}</span>{suggestion.action && <button onClick={() => { setScoreDetailOpen(false); openApply(suggestion); }} className="shrink-0 rounded-md bg-[#fe6147] px-3 py-1.5 text-xs font-bold text-white">AIで適用</button>}</div>)}</div><div className="mt-6 flex justify-end"><button className="btn-ghost" onClick={() => setScoreDetailOpen(false)}>閉じる</button></div></div></div>}
