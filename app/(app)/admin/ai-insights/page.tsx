...
const KPI_CONFIG = [
  ...
  ["診断完了率", "diagnosisCompletionRate", "完了率", "bg-orange-50 text-[#c53f2b]"],
...
export default function AiInsightsPage() {
  ...
  return <div className="ai-report mx-auto max-w-[1480px] px-4 py-6 text-slate-800">
    ...
    <div className="no-print mb-5 flex flex-col gap-3 border-b border-slate-200 pb-5 sm:flex-row sm:items-center sm:justify-between">
      <div><h1 className="flex items-center gap-2 text-2xl font-bold tracking-tight"><Bot className="h-6 w-6 text-[#fe6147]" />AIコンサル・分析</h1><p className="mt-1 text-sm text-slate-500">データから離脱要因・需要・体験予約の伸ばし方を提案します。</p></div>
    ...
    {!data ? <div className="report-panel rounded-xl border border-slate-200 bg-white p-8 text-sm text-slate-500">分析データを取得できませんでした。</div> : <>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_1.15fr_1.15fr_.85fr]">
        ...
        <section className="report-panel overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-100 bg-gradient-to-r from-orange-50 to-white px-5 py-4"><h2 className="flex items-center gap-2 font-bold"><span className="grid h-7 w-7 place-items-center rounded-md bg-[#fe6147] text-sm text-white">2</span>需要トレンドの可視化</h2><p className="mt-1 text-xs text-slate-500">今、どんなニーズが増えているかを把握</p></div><div className="p-4"><p className="mb-3 text-xs font-semibold text-slate-600">ジャンル・目的別の希望割合</p>{data.demand.length ? <div className="space-y-3">{data.demand.map((item, index) => <div key={item.label}><div className="mb-1 flex justify-between gap-2 text-xs"><span className="truncate text-slate-600">{item.label}</span><span className="shrink-0 font-bold">{item.count}人 {item.change != null && <em className={item.change >= 0 ? "ml-1 not-italic text-emerald-600" : "ml-1 not-italic text-rose-600"}>{item.change >= 0 ? "+" : ""}{item.change}%</em>}</span></div><div className="h-2.5 rounded-full bg-slate-100"><div className={["h-2.5 rounded-full", ["bg-[#fe6147]","bg-blue-500","bg-emerald-500","bg-amber-500","bg-cyan-500","bg-sky-400"][index % 6]].join(" ")} style={{ width: `${Math.max(5, item.count / Math.max(1, data.demand[0].count) * 100)}%` }} /></div></div>)}</div> : <p className="py-8 text-center text-sm text-slate-400">診断回答データを蓄積中です</p>}<div className="mt-5 rounded-lg border border-orange-200 bg-orange-50 p-3 text-xs leading-5 text-[#9a3525]"><Lightbulb className="mr-1 inline h-4 w-4 text-[#fe6147]" />増加傾向のジャンルは、LPの冒頭・SNS投稿・チャット初期選択肢で強調すると効果的です。</div></div></section>
        ...
        <section className="report-panel overflow-hidden rounded-xl border border-blue-100 bg-gradient-to-b from-blue-50/70 to-white shadow-sm"><div className="border-b border-blue-100 px-5 py-4"><h2 className="flex items-center gap-2 font-bold text-blue-900"><BrainCircuit className="h-5 w-5" />改善提案レポート</h2><p className="mt-1 text-xs text-blue-700">データから次の打ち手を具体化</p></div><div className="space-y-3 p-3">{data.suggestions.slice(0, 4).map((item, index) => <article key={index} className="rounded-lg border border-white bg-white p-3 shadow-sm"><div className="flex gap-2"><span className={`mt-0.5 h-2.5 w-2.5 shrink-0 rounded-full ${item.priority === "high" ? "bg-rose-500" : item.priority === "medium" ? "bg-amber-400" : "bg-emerald-500"}`} /><div><h3 className="text-sm font-bold text-slate-800">{item.title}</h3><p className="mt-1 text-xs leading-5 text-slate-600">{item.detail}</p><Link href={item.href} className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-blue-700 hover:underline">設定を確認 <ArrowUpRight className="h-3 w-3" /></Link></div></div></article>)}</div><div className="mx-3 mb-3 rounded-lg bg-blue-600 p-3 text-center text-xs font-semibold text-white">改善提案の実行支援も可能です</div></section>
      </div>
