...
export default function AiInsightsClient({ initialInsight = null }: { initialInsight?: Insight | null }) {
  ...
  const funnelOutline = useMemo(() => {
    ...
    return {
      top: stages.map(({ x, top }) => `${x},${top}`).join(" "),
      bottom: stages.map(({ x, bottom }) => `${x},${bottom}`).join(" "),
      edges: stages,
    };
  }, [data, funnelMax]);
