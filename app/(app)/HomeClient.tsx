...
export default function HomeClient({ initialDashboard = null }: { initialDashboard?: DashboardResponse | null }) {
  ...
  // ダッシュボードデータ
  const [qaKpis, setQaKpis] = useState<KPI[]>(initialDashboard?.qaKpis ?? []);
  const [diagnosisKpis, setDiagnosisKpis] = useState<KPI[]>(initialDashboard?.diagnosisKpis ?? []);
  const [setup, setSetup] = useState<SetupItem[]>(initialDashboard?.setup ?? []);
  const [activities, setActivities] = useState<Activity[]>(initialDashboard?.activities ?? []);
  const [system, setSystem] = useState<SystemInfo | null>(initialDashboard?.system ?? null);
  const [recentConversions, setRecentConversions] = useState<ConversionUser[]>(initialDashboard?.recentConversions ?? []);
  const [hasSessionLogs, setHasSessionLogs] = useState(Boolean(initialDashboard?.hasSessionLogs));
  ...
  useEffect(() => {
    if (status === "authenticated") {
      if (!initialDashboard || range !== 7) fetchDashboard();
      fetchDropoff();
